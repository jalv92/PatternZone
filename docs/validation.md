...
- [ ] **6a. Intraday pivot zones (Amendment 8). Playback, DrawZones on.**
      With `UseIntradayPivots` on, replay a session and confirm in order: pivot
      bands **appear** on the chart (OrangeRed above, DodgerBlue below, distinct
      from the SlateGray session bands) only after a level has been touched
      `PivotMinTouches` times; a band **stops growing and greys** when price
      closes cleanly through it; and a pattern whose extremes sit on a pivot band
      but on NO session/round level **enters**, with `DrawRejectedPatterns` on so
      the reason chain is auditable. Then flip `UseIntradayPivots` off and confirm
      the chart has no pivot bands at all and the strategy still trades the
      session/round levels exactly as before.

- [ ] **6e. One trade per instrument (Amendment 9). Two strategies, one instrument.**
      Apply PatternZone twice to the same instrument on the same account with
      NON-overlapping windows (robot 1 early, robot 2 later), `BlockIfInstrumentBusy`
      on. Let robot 1 take a trade that is still open when robot 2's window opens
      and robot 2 gets a signal: robot 2 must print `entry skipped — instrument
      busy: <qty> <instrument> <side> open on the account` and take NOTHING until
      robot 1 is flat. Then confirm the release: once flat, robot 2's next signal
      enters normally. Second pass with the box unticked — robot 2 enters on top,
      which is the pre-amendment behavior. A manual position works as the same
      test if you would rather not run two strategies.
...
- [ ] **6d. Account-wide survives a restart, resets on the day, and leaves no ghost.**
      ...
      broadcast outlives a strategy restart on purpose). Then breach again and read
      the breakdown in the lockout line: it must show **ONE entry per instrument**,
      not two. Two entries for one instrument means the retired instance's key was
      not dropped and the group is double-counting a phantom. The next RTH open must
...
- [ ] **A PAIRED run with `UseIntradayPivots` = false (Amendment 8).** Pivot
      zones ship ON and widen permission, so a single run cannot attribute its
      result to them. Run the frozen configuration twice — pivots on, pivots off
      — and report both. Without the paired run any lift (or loss) is
      unattributable and the gate table means nothing for this amendment.
...
| The **instrument-busy check is not transactional** — two robots entering in the same instant can still both get in | Amendment 9. The guard reads the account's positions and then submits; nothing holds a lock across those two steps, and nothing could without blocking the platform's own threads. A sibling robot whose fill lands between our check and our submit is invisible to us. The window is one bar wide at worst and the sequential-window setup this was built for never opens it. The mirror direction is also live: the same window can read our OWN just-closed position as busy — a fill still reflected in `Account.Positions` while the strategy already reads flat — so we skip an entry we could have taken. That is the safe direction, and the skip consumes a session slot per the convention. Recorded, not solved: closing it properly needs an order-level reservation NT8 does not offer. |
| **In the Strategy Analyzer the guard is effectively inert** | Amendment 9. The account there is virtual and the run is single-strategy, so the only position the collection can hold is this strategy's own — and the check runs only after our own flat guard has passed, at which point there is nothing to find. Realtime/Playback behavior is the real behavior. Not verified by a run: reasoned from NT8's definition of the Account object as "the real-world or simulation Account configured for the strategy". An OPTIMIZATION is the exception to watch, and it is the same hazard Amendment 6 documents for `AccountWide`: many iterations run against one virtual account, so if their positions surface in a shared `Account.Positions` they would block each other exactly as they would pool P&L. `BlockIfInstrumentBusy` defaults **ON** where `AccountWide` defaults off, so the optimizer guidance is the same for both — **toggle it OFF for optimizations too**. |
| Pivot zones born on bars **2-14** of the pivot series carry systematically **tighter** bands | Amendment 8, inherited from PullbackZone. The half-width is frozen at the pivot series' Wilder ATR on the reveal bar, and that ATR is still seeded (a running mean until bar 14) — so an early-session zone gets a narrower box than the same pivot would produce later. The zone is not wrong, it is just tighter, and it never widens afterwards because a zone's edges cannot drift. Consequence for testing: the first zones of a fresh chart load are the strictest ones. |
| **Pivot zones move only on CLOSED pivot-series bars** — up to `PivotSeriesMinutes` of lag against the chart | Amendment 8. The fold loop only consumes pivot bars whose time is at or before the current chart bar's close; that guard is what keeps the engine free of lookahead, and the cost is that a zone born, touched or killed inside the current 5-minute bar is not visible until that bar closes. On a 150-tick chart this means the permission set can be up to five minutes stale — deliberate, and the same trade PullbackZone makes. |
| Under a **Playback connection**, applying the strategy CLEARS a previous run's account-wide record; live, the breach survives a re-enable | Bug 2026-08-12. The record is keyed (account, **tape** day) and the breach outliving a re-enable is deliberate — live, a daily limit you can clear with a checkbox is not a limit (exercise 6d). But in Playback the same tape day comes round on every replay, so a fresh application would inherit the previous run's breach and refuse to trade the whole day. On a Playback connection (`Account.Provider == Provider.Playback`) a joining instance therefore drops the account's record — but **only when no live contributor is left**, so two Playback charts still share one governor. It says so in the Output when it happens. Live and Sim are untouched. |
| A **hard crash or process kill** leaves one ghost contributor in the shared record until the day rolls | Amendment 6. A retiring instance drops its own key at `State.Terminated`, which NT8 raises for every ordinary teardown — disable, re-enable, parameter change, recompile, reconnect. A kill -9 or a platform crash raises nothing, so that instance's last published P&L stays in the total until the next session open builds a fresh record. The tell is in the breach log's breakdown: **two lines for one instrument**. It cannot be cleared mid-day by design — the alternative, wiping the record on load, is what would let a disable/re-enable clear a live breach. |
| In **ATM mode**, account-wide's open-P&L term comes from `GetAtmStrategyUnrealizedProfitLoss`, which is **unexercised** | Amendment 6. An ATM position is invisible to `Position` (design.md item 12), so `Position.GetUnrealizedProfitLoss` reads 0 during a live ATM trade and the shared total would price an open ATM position at zero without this getter. It is wrapped in the same `try`/`catch` as every other ATM getter in the file, so a throw degrades to realized-only rather than killing the bar, and a returned NaN degrades to realized-only too (publishing 0 there would drop this instance's realized loss from the group total and fire an account-wide loss limit late) — but no Playback run has exercised it yet. Exercise 6c/6d in ATM mode is what would prove it; until then, treat an ATM + account-wide combination as unvalidated. |
